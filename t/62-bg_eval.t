...
SKIP: {
  ...
  if (!Forks::Super::Config::CONFIG("YAML")) {
    skip "YAML not available, skipping bg_eval tests", 35;
  }
...
ok($Forks::Super::LAST_JOB->{_is_bg} > 0, "\$Forks::Super::LAST_JOB marked bg");
my $p = waitpid -1, 0;
...
ok($t0 >= 2.95 && $t <= 3.85, "scalar bg_eval took ${t}s ${t0}s expected ~3s");
$$x = 19;
...
my $t2 = Time();
($t,$t0) = ($t2-$t,$t2-$t0);
ok($t0 >= 1.95 && $t <= 3.57, "listref bg_eval took ${t0}s ${t}s expected ~2s"); ### 25 ### was 3.25 obs 3.56
